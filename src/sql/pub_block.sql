...
CREATE TABLE IF NOT EXISTS `pub_block_category` (
  ...
  `is_enable` tinyint(1) UNSIGNED NOT NULL DEFAULT '1' COMMENT '页面区块启用状态',
  `src` varchar(200) NOT NULL DEFAULT '' COMMENT 'type为image-link时，为图片地址',
  `x_flag` varchar(20) NOT NULL DEFAULT '' COMMENT 'type为content的在线编辑器标识符',
...
CREATE TABLE IF NOT EXISTS `pub_block_option` (
  ...
  `sort_order` tinyint(4) UNSIGNED NOT NULL DEFAULT '0' COMMENT '排序',
  `is_open` tinyint(1) UNSIGNED NOT NULL DEFAULT '1' COMMENT '是否对管理开放',
