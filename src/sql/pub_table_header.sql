...
-- 对程序代码，提供统一出口 \Tools\TableHeader::getHeader($key);
...
CREATE TABLE IF NOT EXISTS `pub_header_category` (
  `key` varchar(100) NOT NULL COMMENT '表头索引或标志，全站唯一',
...
CREATE TABLE IF NOT EXISTS `pub_header_option` (
  ...
  `key` varchar(100) NOT NULL COMMENT '所属表头分类（来自header_category）',
  ...
  `css_class` varchar(30) DEFAULT 'text-center' COMMENT '元素显示位置[text-left:靠左;text-center:居中;text-right:靠右;]',
